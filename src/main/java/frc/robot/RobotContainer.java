...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    findNote =  arm.setIntakePosition().andThen(intake.intakeOn()).andThen(new DriveToGamePiece(drivetrain, intakeCamera, arm))
        .until(() -> SmartDashboard.getBoolean("noteLoaded", false))
        .withTimeout(5)
        .alongWith(runOnce(() -> SmartDashboard.putBoolean("autoControlled", true)));

    autofindNote = arm.setIntakePosition().andThen(intake.intakeAutoStop()).andThen(new DriveToGamePiece(drivetrain, intakeCamera, arm))
